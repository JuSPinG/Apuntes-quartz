...
replace_in_file "^#\?Port[[:space:]]\+.*" "Port $NEW_SSH_PORT" "$SSH_CONFIG"
replace_in_file "^#\?PermitRootLogin[[:space:]]\+.*" "PermitRootLogin no" "$SSH_CONFIG"
replace_in_file "^#\?PermitEmptyPasswords[[:space:]]\+.*" "PermitEmptyPasswords no" "$SSH_CONFIG"
replace_in_file "^#\?MaxAuthTries[[:space:]]\+.*" "MaxAuthTries 3" "$SSH_CONFIG"
replace_in_file "^#\?LoginGraceTime[[:space:]]\+.*" "LoginGraceTime 30" "$SSH_CONFIG"
replace_in_file "^#\?ClientAliveInterval[[:space:]]\+.*" "ClientAliveInterval 300" "$SSH_CONFIG"
replace_in_file "^#\?ClientAliveCountMax[[:space:]]\+.*" "ClientAliveCountMax 0" "$SSH_CONFIG"
replace_in_file "^#\?X11Forwarding[[:space:]]\+.*" "X11Forwarding no" "$SSH_CONFIG"
replace_in_file "^#\?Banner[[:space:]]\+.*" "Banner $BANNER_FILE" "$SSH_CONFIG"
