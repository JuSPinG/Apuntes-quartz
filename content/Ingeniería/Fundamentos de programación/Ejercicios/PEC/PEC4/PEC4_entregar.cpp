...
typedef int TipoNumerosApartamentos[MAX_Apartamentos]; /* Pod�ra haber usado una lista enlazada en vez de una matriz,
                                                          pero algunas partes del programa se volv�an demasiado complejas */
...
/* los subprogramas mantienen el orden de declaraci�n de interfaz */
...
/*--  Funci�n de ImprimiMenuInicio, sirve para imprimir el men� todo junto a la vez, �til para la limpieza visual --*/
char TipoInterfaz::ImprimirMenuInicio() {
  ...
  return opcion; /* Retorna la opci�n que el usuario elije */
}
...
/*--  Funci�n de ComprobarDia, nos sirve para saber si el d�a puede existir o no  --*/
...
/*--  Funci�n de CoomprobarMes, importante para no seleccionar meses inexistentes  --*/
...
/*--  Funci�n de CoomprobarAnno, as� se comprueba que el a�o sea correcto  --*/
...
/*--  Funci�n de ComprobarBisiesto, muy necesario para ajustar los desfases  --*/
bool TipoFecha::ComprobarBisiesto(TipoAnno anno) {
  return (anno % 4 == 0 && anno % 100 != 0) || (anno % 400 == 0);
}

/*--  Funci�n de ObtenerLongitudMes, si es necesario calcular distancias entre fechas, es necesario abstraerlo por a�os, meses, y d�as  --*/
...
/*--  Funci�n de ObtenerLongitudAnno, se usa para saber cu�ntos d�as tiene un a�o  --*/
int TipoFecha::ObtenerLongitudAnno(TipoAnno anno) {
  return 365 + int(ComprobarBisiesto(anno)); /* Si es true, se pasa a int, donde equivale a 1, as� se simplifica un poco el subprograma */
}

/*--  Funci�n de ObtenerDias, esto sirve para pasar de una fecha a su n�mero de d�as, partiendo desde el 1/1/1601  --*/
...
/*--  Funci�n de ObtenerFecha, igual que la funci�n de antes pero al reves, se introduce un n�mero y sale una fecha  --*/
TipoFecha TipoFecha::ObtenerFecha(int dias) {
    int contadorDias = 0;
    int diasMes;
    TipoFecha fecha;

    /* A continuaci�n, se anidan varios for para poder extraer la fecha correcta */
...
typedef struct TipoConjuntoEdificios {
  TipoEdificio edificios[MAX_Edificios]; /* Aqu� declaro una matriz de edificios, esto resulta muy c�modo a la hora de desarrollar el programa */
  ...
  /* De forma orientativa, cada una de estos procedimientos corresponden a cada una de las posibles respuestas del cliente en el men� */
...
/*--  Funci�n de ComprobarIdentificador, es necesaria para comprobar un identificador correcto  --*/
...
/*--  Funci�n de ComprobarDisponibilidad, como su nombre indica, comprueba cu�ntos apartamentos est�n disponibles para una fecha y edificio concretos  --*/
...
/*--  Procedimiento de EditarEdificio, esta funci�n crea una edificio y hace ciertas comprobaciones para que todo est� correcto  --*/
void TipoConjuntoEdificios::EditarEdificio(TipoConjuntoEdificios &edificio) {
  ...
  if (!ComprobarIdentificador(identificador)) { /* Comprobaci�n de si identificador es posible */
    throw 1;
  }
  ...
  /*-- Se ejecuta el men� de selecci�n --*/
  interzaf.Espaciar(5);
  ...
  if (toupper(respuesta) == 'S') { /* Aqu� se sobreescribe el edificio, me apoyo de un edificio auxiliar para poder guardar los datos */
...
void TipoConjuntoEdificios::ListarEdificios(TipoConjuntoEdificios edificio) {
  ...
  for (int i = 0; i < MAX_Edificios; i++) {
    if (edificio.edificios[i].iniciado == true && edificio.edificios[i].baja == false) { /* Aqu� se comprueba si el edificio existe, para evitar problemas de obtener datos que no son */
  ...
  if (hayEdificios == false) { /* As� se presenta que no hay edificios */
    printf("No hay edificios disponibles\n");
  }
  printf("\n\n");
}

/*--  Procedimiento de ConsultarReservas, consulta las reservas que hay disponibles, se usa la funci�n ComprobarDisponibilidad declarada anteriormente  --*/
void TipoConjuntoEdificios::ConsultarReservas(TipoConjuntoEdificios edificio) {
  int longitud;
  int disponibilidadCorrecta = 0; /* Esto nos servir� para saber si el edificio seleccionado tiene apartamentos disponibles */
  ...
  /*-- Impresi�n del men� y manejo de los datos obtenidos --*/
  printf("Apartamentos Disponibles: \n\n");
  ...
  identificador--; /* En el programa, es una pr�ctica habitual, obtener un dato y tratarlo */

  if (!ComprobarIdentificador(identificador)) { /* Comprobaci�n del identificador */
    throw 1;
  }

  edificioSeleccionado = edificio.edificios[identificador];

  if (edificioSeleccionado.baja) { /* Comprobaci�n de que el edificio se encuentre iniciado */
    throw 2;
  }

  /* Continuaci�n del men� */
  interfaz.Espaciar(5);
  ...
  fecha.mes = TipoMes(int(fecha.mes) - 1); /* M�s tratamiento de datos, el mes enero se corresponde con el 0, por eso se resta 1 */
  ...
  for (int i = 0; i <= longitud; i++) {
    ...
    /* Los siguientes apartados detectan los apartamentos libres en un d�a espec�fico, y si uno es mayor que el otro, ambos se establecen en el menor */
    ...
    if (apartamentosLibres.N_apartamentoBasico + apartamentosLibres.N_apartamentoNormal + apartamentosLibres.N_apartamentoLujo > 0) {
      disponibilidadCorrecta++; /* Comprobaci�n de que todo est� correcto, de que haya apartamentos disponibles */
    }
  }

  /* Por �ltimo, se imprimen los edificios disponibles, si hay */
...
/*--  Procedimiento de HaceReserva, aqu� el programa obtiene e inserta los datos en el edificio  --*/
void TipoConjuntoEdificios::HacerReserva(TipoConjuntoEdificios &edificio) {
  ...
  /* Tratamiento de datos y comprobaci�n del dato identificador */
  identificador--;
  ...
  /* Este fragmento se encarga de cambiar seleccionar los apartamentos en funci�n de la letra */
  ...
  for (int i = 0; i < longitud; i++) { /**/
    ...
    while (cursor != NULL) {
      ...
      N_deReserva++; /* Esto servir� m�s adelante para saber qu� n�mero de reserva ocupa una reserva */
        if (cursor->fecha.mes == fechaAux.mes && cursor->fecha.dia == fechaAux.dia && cursor->calidadHabitacion == tipoApartamento) {
          ...
          for (int i = 0; i < MAX_Apartamentos && !habitacionOcupada; i++) { /* La raz�n de este nuevo for radica en la necesidad de no procesar elementos repetidos */
  ...
  /* Se recorre la lista enlazada hasta su �ltimo elemento */
  cursor = edificio.edificios[identificador].informacionReservado;
  ...
  } else { /* Es decir, si s� hay reservas */
    for (int i = 0; i < contador; i++) {
      for (int j = 0; j < contador; j++) {
        if (habitacionesOcupadas[j] == N_habitacionSeleccionada) {
          N_habitacionSeleccionada++; /* Esto sirve para ver qu� habitaciones est�n seleccionadas, proceso que se omite si no hay reservas previas */

          if (N_habitacionSeleccionada >= N_apartamentos) {
            throw 7; /* En el caso de que la habitaci�n salga del rango */
          }
        }
      }
    }
  ...
  /* Peque�o arreglo para imprimir los identificadores */
  ...
  /* Peque�o arreglo para imprimir el n�mero de habitaci�n */
...
/*--  Procedimiento de ImprimirFecha, un peque�o �til para imprimir el inicio del calendario  --*/
...
/*--  Procedimiento de ImprimirSeparador, es similar al procedimiento Espaciar(), pero este proviene
      de otro programa, se incluy� porque se especific� la reutilizaci�n de c�digo  --*/
...
/*--  Procedimiento de ImprimirSiglasSemana, peque�o �til para imprimir el formato del calendario  --*/
...
/* Peque�a no: Esta es la parte m�s compleja del programa */
void CalendarioMes::GenerarCalendario(TipoConjuntoEdificios edificio) {
  ...
  /* Esta secci�n maneja los datos en la referencia */
  if (referencia[0] == '0') {
  ...
  /* Manejo de la letra, para pasarla a n�mero */
  switch (tipoApartamentoLetra) {
    case 'B':
      tipoApartamento = 0;
      break;
  ...
  /* Comprobar que la fecha est� bien */
  ...
  fecha.dia = 0; /* Como el d�a no se especifica, se establece en 0 para que no estorbe */

  /* Men� en el que se imprimen algunos datos */
  printf("\n");
  interfaz.Espaciar(10);
  ...
  /* A continuaci�n, se imprime el calendario */
  printf("\n");
  interfaz.Espaciar(15);
  ImprimirSeparador(27, '-');

  /* Obtenci�n del d�a de inicio, para calcular el desfase semanal de calendario */
  diaInicio = (fecha.ObtenerDias(fecha)) % 7;
  semanasNecesarias = (fecha.ObtenerLongitudMes(fecha.mes, fecha.anno) + diaInicio + 6) / 7; /* Con esto se calculan cu�ntas semanas se van a necesitar */
  fecha.dia = -diaInicio; /* Peque�o arreglo para saber cu�ndo empezar a imprimir */

  for (int i = 0; i < semanasNecesarias; i++) { /* Bucle para establecer cu�ntas l�neas de semana se necesitan */
    printf("\n");
    interfaz.Espaciar(15);
    for (int j = 0; j < LongitudSemana; j++) {
      ...
      cursor = edificio.edificios[identificador].informacionReservado;
      reservado = false; /* Variable un poco auxiliar para no hacer el bucle m�s de una vez */
      ...
      while (cursor != NULL && !reservado) { /* Se recorre el cursor, hasta que finalmente se encuentra un �espacio libre� */
        ...
        && cursor->N_Habitacion == N_habitacionSeleccionada) {
          reservado = true;
          contadorDias++;
          printf("Re"); /* Se imprime la marca "Re" para indicar que el d�a est� reservado */

          diasReservados++; /* Usado para el conteo posterior */
        }

        cursor = cursor->siguiente; /* Se avanza una posici�n en el cursor */
      }

      if (!reservado) { /* En el caso de que no haya una reserva */
        /* Este apartado se encarga de cuando los d�as del mes anterior llegan a su fin */
        if (i == 0 && j < diaInicio - 1) {
          printf("  "); /* Es imprimen espacios para rellenar */

        } else if (contadorDias > fecha.ObtenerLongitudMes(fecha.mes, fecha.anno) || repetirFinal) { /* Cuando los d�as superan el mes */

          printf("  "); /* M�s espacios para rellenar */
          contadorDias++;
          repetirFinal = true; /* Para que se siga repitiendo hasta que finalice, si no, cambia de mes */

        } else { /* Si el d�a est� dentro del mes */
          printf("%2d", contadorDias); /* Imprime el d�a del mes */
          contadorDias++;
        }
      }
  ...
  while (cursor != NULL) {
    if (cursor->fecha.anno > 0) {
      contadorDias = 0; /* Se reutiliza la variable, realmente vuelve a contar d�as */
      ...
      /* Se recorre el cursor hasta alcanzar la misma reserva (de n�mero) */
      while (cursor->siguiente != NULL && cursor->siguiente->N_deReserva == reservaActual) {
        contadorDias++;
        cursor = cursor->siguiente;
      }

      /* Se imprime la informaci�n de la reserva */
  ...
  printf("\nTotal dias reservados del mes: %d dias", diasReservados); /* As� se saben los d�as que se han reservado */
  printf("\nTotal dias libres del mes: %d dias", fecha.ObtenerLongitudMes(fecha.mes, fecha.anno) - diasReservados); /* Y se restan a los d�as totales */
  printf("\n\n");
}
...
int main() {
  ...
  while (respuesta != 'S') {
    respuesta = toupper(interfaz.ImprimirMenuInicio()); /* Llamada al men� del inicio */
    printf("\n");

    switch (respuesta) { /* Se eval�an las posibles respuestas */
