...
do_backup "$LOGROTATE_CONF"

LOGROTATE_SYSLOG="/etc/logrotate.d/syslog"
do_backup "$LOGROTATE_SYSLOG"
...
replace_in_file "^#\?compress" "compress" "$LOGROTATE_CONF"
replace_in_file "^#\?weekly" "weekly" "$LOGROTATE_CONF"
replace_in_file "^rotate[[:space:]]\+.*" "rotate 12" "$LOGROTATE_CONF"
replace_in_file "^#\?create" "create" "$LOGROTATE_CONF"

# 6. Verificación de logrotate
log_info "Validando configuración de logrotate..."
if logrotate -f $LOGROTATE_CONF; then
