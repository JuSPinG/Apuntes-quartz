...
echo -e "${CYAN}------------------------------------------------------------------"
echo -e "  CCN-STIC-610 - Gestión de Servicios del Sistema en RHEL"
echo -e "------------------------------------------------------------------"
echo
...
enable_service() {
  ...
  if ! systemctl list-unit-files | grep -qw "${servicio}.service"; then
    echo -e "${CYAN}[i] $descripcion no está instalado.${NC}"
    return
  fi

  if systemctl is-enabled "$servicio" &>/dev/null; then
    echo -e "${GREEN}[✔] $descripcion ya está habilitado.${NC}"
  else
    if systemctl enable --now "$servicio" &>/dev/null; then
      echo -e "${GREEN}[✔] $descripcion habilitado.${NC}"
    else
      echo -e "${RED}[✗] Error al habilitar $descripcion.${NC}"
    fi
  fi
}
...
disable_service() {
  ...
  if ! systemctl is-enabled "$servicio" &>/dev/null; then
    echo -e "${GREEN}[✔] $descripcion ya está deshabilitado.${NC}"
  else
    if systemctl disable --now "$servicio" &>/dev/null; then
      echo -e "${GREEN}[✔] $descripcion deshabilitado.${NC}"
    else
      echo -e "${RED}[✗] Error al deshabilitar $descripcion.${NC}"
    fi
  fi
}
...
mask_service() {
  ...
  if systemctl is-enabled "$servicio" 2>/dev/null | grep -q masked; then
    echo -e "${GREEN}[✔] $descripcion ya está enmascarado.${NC}"
  else
    if systemctl mask "$servicio" &>/dev/null; then
      echo -e "${GREEN}[✔] $descripcion enmascarado.${NC}"
    else
      echo -e "${RED}[✗] Error al enmascarar $descripcion.${NC}"
    fi
  fi
}
...
unmask_service() {
  ...
  if systemctl is-enabled "$servicio" 2>/dev/null | grep -q masked; then
    if systemctl unmask "$servicio" &>/dev/null; then
      echo -e "${GREEN}[✔] $descripcion desenmascarado.${NC}"
    else
      echo -e "${RED}[✗] Error al desenmascarar $descripcion.${NC}"
      return
    fi
  else
    echo -e "${GREEN}[✔] $descripcion ya está desenmascarado.${NC}"
  fi
